...
export default function NoticeDetail() {
    ...
    const handleSave = async () => {
        ...
        if (!content) return setAlertMessage({ message: "내용을 입력해주세요.", severity: "error" });
        ...
        <Layout>
            <Box p={4}>
                ...
                <Stack direction="row" spacing={2} justifyContent="flex-end">
                    <Button variant="outlined" color="error" size="large" onClick={() => setShowDeleteConfirm(true)} disabled={isProcessing} sx={{ py:1.5, px:4, borderRadius:2, marginRight:'auto' }}>삭제</Button>
                    <Button variant="contained" color="primary" size="large" onClick={() => router.push("/notice")} disabled={isProcessing} sx={{ py:1.5, px:4, borderRadius:2 }}>목록</Button>
                    <Button variant="contained" color="success" size="large" onClick={handleSave} disabled={isProcessing || !title.trim() || !isEditorReady} sx={{ py:1.5, px:4, borderRadius:2 }}>{isProcessing ? "저장 중..." : "저장"}</Button>
                </Stack>
