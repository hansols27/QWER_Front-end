...
const SmartEditor = dynamic<any>(
    () => import("@components/common/SmartEditor").then(m => m.default),
    {
        ssr: false,
        loading: () => (
...
export default function NoticeDetail() {
    ...
    const fetchNotice = useCallback(async () => {
        ...
        try {
            ...
            setAlertMessage({
                message: extractErrorMessage(err, "공지사항 로드 실패"),
                severity: "error"
            });
        } finally {
            setLoading(false);
    ...
    const handleSave = async () => {
        ...
        if (!isEditorReady) {
            setAlertMessage({
                message: "에디터 로딩 중입니다. 잠시 후 시도해주세요.",
                severity: "warning"
            });
            return;
        }
        ...
        if (!editor || typeof editor.getContent !== "function") {
            ...
            setAlertMessage({
                message: "에디터 초기화 오류. 새로고침 후 다시 시도해주세요.",
                severity: "error"
            });
            return;
        }
        ...
        const isQuillEmpty = content === "" || content === "<p><br></p>";
        ...
        try {
            ...
            setAlertMessage({
                message: extractErrorMessage(err, "수정 실패"),
                severity: "error"
            });
        } finally {
    ...
    const executeDelete = async () => {
        ...
        try {
            ...
            setAlertMessage({
                message: extractErrorMessage(err, "삭제 실패"),
                severity: "error"
            });
        } finally {
            setIsProcessing(false);
    ...
    const handleDelete = () => {
        if (!isProcessing) setShowDeleteConfirm(true);
    };
        ...
        return (
            <Layout>
                <Box p={4}>
                    ...
                    <Button onClick={() => router.push("/notice")} variant="contained" sx={{ mt: 2 }}>
                        목록으로 이동
                    </Button>
                </Box>
            </Layout>
    ...
    return (
        <Layout>
                ...
                <Card sx={{ p: 3, borderRadius: 2, boxShadow: 3 }}>
                    <Stack spacing={3}>
                        ...
                        <Stack direction="row" spacing={2} alignItems="center">
                            <Select
                                value={type}
                                onChange={(e: SelectChangeEvent<NoticeType>) =>
                                    setType(e.target.value as NoticeType)
                                }
            ...
            <Dialog open={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)}>
                ...
                <DialogActions>
                    <Button onClick={() => setShowDeleteConfirm(false)} disabled={isProcessing}>
                        취소
                    </Button>
                    <Button
