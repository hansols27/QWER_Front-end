...
// 환경 변수를 사용하여 API 기본 URL을 가져와 상수로 저장합니다.
...
      //API_URL 상수를 사용하여 유효성 검사를 수행합니다.
      if (!NEXT_PUBLIC_API_URL) {
...
    // API_URL 상수를 사용하여 유효성 검사를 수행합니다.
    if (!selectedDate || !NEXT_PUBLIC_API_URL) {
      if (!NEXT_PUBLIC_API_URL) setAlertMessage({ message: "API 주소가 설정되지 않아 저장할 수 없습니다.", severity: "error" });
      return; 
    } 
...
    // API_URL 상수를 사용하여 유효성 검사를 수행합니다.
    if (!editId || !NEXT_PUBLIC_API_URL) {
      if (!NEXT_PUBLIC_API_URL) setAlertMessage({ message: "API 주소가 설정되지 않아 삭제할 수 없습니다.", severity: "error" });
      return;
    }
...
              variant="contained" 
              onClick={saveEvent} 
              disabled={loading || !NEXT_PUBLIC_API_URL || !title} // API_URL 상수를 사용하여 비활성화 조건 설정
