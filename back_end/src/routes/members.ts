...
// ----------------------
// 멤버 저장 (이미지 + 기존 URL 유지 + 유효성 체크)
// ----------------------
router.post("/save", upload.array("images"), async (req, res) => {
  try {
    const { members } = JSON.parse(req.body.data);
    ...
    for (const id of MEMBER_IDS) {
      ...
      const snsCount = Object.values(member.sns || {})
    ...
    // Firestore 저장 (기존 URL 유지 + 새로 업로드된 이미지 추가)
    for (const member of members) {
      const imagesFromUpload = uploadedUrls[member.id] || [];
      const contentsWithImages = member.contents.map((c: any) => {
